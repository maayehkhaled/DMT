...
import com.qpros.quanta.reporter.BasicFileReporter;

abstract class QuantaObservable implements ReportService {
     ...
     * This setting allows setting test with your own variables and prevent update by Quanta.
     * As of today, with this enabled, Quanta does not use time-stamps for tests at the time
     * they were created
     */
     ...
     * A collection of tests arranged by category
     */
    private TestAttributeTestContextProvider<SubCategory> subCategoryContext = new TestAttributeTestContextProvider<>();
    ...
    /**
     * A list of all {@link QuantaReporter} reporters started by the <code>attachReporter</code>
     * method
     */
    private List<QuantaReporter> reporterList = new ArrayList<>();

    /**
     * Any logs added by to the test runner can be added to Quanta
     *
     * <p>
     * TestNG Example:
     *
     ...
     *
     * <p>
     * Informing Quanta of any logs added:
     * <pre>
     ...
     */
    protected void register(QuantaReporter reporter) {
     ...
     *
     * @param reporter {@link QuantaReporter} reporter to unsubscribe
     */
    protected void deregister(QuantaReporter reporter) {
        reporter.stop();
     ...
     *
     * @return a list of {@link QuantaReporter} objects
     */
    protected List<QuantaReporter> getReporterCollection() {
        return reporterList;
    }
    ...
    private void removeTestTestAttributeContext(Test test) {
        ...
        if (test.hasSubCategory()) {
            subCategoryContext.removeTest(test);
        }
        if (test.hasAuthor()) {
    ...
    private void refreshStatusList(List<Test> list) {
        ...
        list.stream().map(Test::getStatus).distinct().collect(Collectors.toList()).forEach(x -> statusMap.put(x, false));
    ...
    synchronized void assignCategory(Test test, Category category) {
        reporterList.forEach(x -> {
            ...
            if (!category.subCategoryList.isEmpty()) {
                category.subCategoryList.forEach(sub -> x.onSubCategoryAssigned(test, sub));
            }
        });
    }
    /**
    ...
    synchronized void addScreenCapture(Test test, ScreenCapture screenCapture) throws IOException {
        for (QuantaReporter r : reporterList) {
    ...
    private synchronized void collectRunInfo() {
        ...
        for (Test test : testList) {
            ...
            if (test.hasSubCategory()) {
                test.getSubCategoryContext().getAll().forEach(x -> subCategoryContext.setAttributeContext((SubCategory) x, test));
            }
            if (test.hasAuthor()) {
    ...
    private void copyNodeAttributeAndRunTimeInfoToAttributeContexts(Test node) {
        ...
        if (node.hasCategory()) {
            node.getSubCategoryContext().getAll().forEach(x -> subCategoryContext.setAttributeContext((SubCategory) x, node));
        }
        if (node.hasAuthor()) {
    ...
    private synchronized void notifyReporters() {
        ...
        ReportAggregates reportAggregates =
                        new ReportAggregatesBuilder().setAuthorContext(authorContext).setCategoryContext(categoryContext).setSubCategoryContext(subCategoryContext).setDeviceContext(deviceContext)
                                        .setExceptionContext(exceptionContextBuilder).setReportStatusStats(stats).setStatusList(statusList).setSystemAttributeContext(systemAttributeContext)
                                        .setTestList(testList).setTestRunnerLogs(testRunnerLogs).setStartTime(reportStartDate).setEndTime(reportEndDate).build();
    ...
    protected void end() {
        flush();
        reporterList.forEach(QuantaReporter::stop);
        reporterList.clear();
    }
